...
Future<void> main() async {
  ...
  runApp(DemoApp(controller: themeController, showController: ShowController()));
}

class DemoApp extends StatelessWidget {
  const DemoApp({super.key, required this.controller, required this.showController});
...
class ShowWidget extends InheritedWidget {
  ShowWidget({required this.showController, required super.child});
  final ShowController showController;

  static ShowWidget of(BuildContext context) {
    final ShowWidget? result = context.dependOnInheritedWidgetOfExactType<ShowWidget>();
    assert(result != null, 'No ShowWidget found in context');
    return result!;
  }

  @override
  bool updateShouldNotify(ShowWidget oldWidget) {
    return showController.value != oldWidget.showController.value;
  }
}
